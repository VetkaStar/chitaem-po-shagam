...
const material = load(path.join(root, 'features/free-practice/material-ready.ts'));
act(m => { m.navigate('syllables', 'read'); m.setLessonMic(true); });
act(() => speechOptions.onReady());
const pendingMaterial = {};
const beforePendingStars = model.stars;
act(() => material.setMaterialPending(pendingMaterial, true));
act(() => speechOptions.onResult({text:model.target.toLowerCase(), result:[{conf:0.95}]}));
assert.equal(model.stars, beforePendingStars, 'pending material cannot accept a spoken result');
assert.notEqual(model.feedback.kind, 'success');
assert.equal(model.lessonMic, true, 'persistence does not clear microphone intent');
act(() => material.setMaterialPending(pendingMaterial, false));
act(() => speechOptions.onResult({text:model.target.toLowerCase(), result:[{conf:0.95}]}));
...
act(() => speechOptions.onPartial(model.target.toLowerCase()));
assert.equal(model.stars, experimentalStars, 'partial hypothesis is not an answer');
act(() => speechOptions.onResult({ text: model.target.slice(0, 1), experimental: true }));
