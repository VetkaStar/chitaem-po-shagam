...
const { taskSection } = await import(
  'data:text/javascript;base64,' + Buffer.from(sectionCode).toString('base64')
);
    ...
    for (const section of [
        ...
        ({ sectionId }) => {
          if (!localStorage.getItem('reading-profile-v1')) {
            localStorage.setItem(
              'reading-profile-v1',
              JSON.stringify({ name: 'Режимы', age: '8', start: sectionId }),
            );
            localStorage.setItem(
              'reading-steps-v3',
              JSON.stringify({
          ...
          Object.defineProperty(window, 'speechSynthesis', {
            ...
            value: {
              ...
              removeEventListener() {},
              speak(utterance) {
                window.sectionTestSpoken.push(utterance.text);
      ...
      const enter = async () => {
        await page
          .locator('.portal-grid')
          .getByRole('button', {
            name: new RegExp('ШАГ ' + section.step + ' ' + section.title),
          })
          .click();
        if (section.id !== 'letters')
          await page
            .locator('.portal-grid')
            .getByRole('button', { name: section.core, exact: true })
            .click();
      ...
      for (const step of route.steps) {
        assert.ok(
          bank.items[step.itemId],
          'reference must point to the supplied task',
        );
      ...
      assert.equal(
        (await page.locator('.answer-material').textContent()).trim(),
        target.trim(),
      );
      assert.equal(
        await page.locator('.answer-material').getAttribute('data-section'),
        section.id,
      );
      assert.equal(
        await page
      ...
      const selectedCore = page.locator(
        section.id === 'letters'
          ? '.app-nav-item[aria-current="page"]'
          : '.app-nav-child[aria-current="page"]',
      );
      assert.equal(await selectedCore.count(), 1);
      assert.ok(
        (await selectedCore.textContent()).includes(
          section.id === 'letters' ? section.title : section.core,
        ),
      );
      ...
      assert.ok(
        (await page.evaluate(() => window.sectionTestSpoken)).includes(target),
      );
      await page.screenshot({
        path: path.join(output, `${width}-${section.id}.png`),
        fullPage: true,
      });
      await page.reload();
      await enter();
      ...
      assert.equal(
        restored.profile.activeInstance.instanceId,
        active.instanceId,
      );
      assert.equal(
        (await page.locator('.answer-material').textContent()).trim(),
      ...
      assert.ok((await selectedCore.textContent()).includes(section.id === 'letters' ? section.title : section.core));
      assert.equal(
        await page.evaluate(
      ...
      completed.push(
        `${width} ${section.id}: authored pool, four tabs, sample label/audio, selected core, reload exact instance`,
      );
      await context.close();
    }
  }
