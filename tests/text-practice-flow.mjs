...
assert.equal(result.progress, 3, 'final fragment confirms prefix without completing line');
...
key = 'experimental-syllables';
target = 'Мама мыла раму.';
flush();
const beforeParts = completes;
speechCallbacks.onPartial('ма'); flush();
assert.equal(result.previewProgress, 2);
assert.equal(result.progress, 0);
speechCallbacks.onResult({text:'ма', experimental:true}); flush();
assert.equal(result.progress, 2);
speechCallbacks.onPartial('ма'); flush();
assert.equal(result.previewProgress, 4);
assert.equal(result.progress, 2);
speechCallbacks.onResult({text:'ошибка', experimental:true}); flush();
assert.equal(result.progress, 2);
assert.equal(result.previewProgress, 0);
speechCallbacks.onResult({text:'ма', experimental:true}); flush();
assert.equal(result.progress, 4);
speechCallbacks.onResult({text:'мыла лишнее', experimental:true}); flush();
assert.equal(result.progress, 4);
speechCallbacks.onResult({text:'мы', experimental:true}); flush();
assert.equal(result.progress, 6);
speechCallbacks.onResult({text:'ла ра', experimental:true}); flush();
assert.equal(result.progress, 10);
assert.equal(completes, beforeParts);
speechCallbacks.onResult({text:'му', experimental:true}); flush();
assert.equal(completes, beforeParts + 1);
console.log('PASS experimental syllables: live preview, repeated parts, errors and final completion');
...
assert(uiMic[2]);
assert(!text(result).includes(item.question));
const beforeRepeatKey = uiMic[1];
click('Повторить задание');
assert.equal(uiMic[0], item.lines[0], 'repeat must restart the entire text');
assert.notEqual(uiMic[1], beforeRepeatKey, 'repeat invalidates previous speech');
assert(uiMic[2], 'repeat preserves the enabled microphone session');
uiMic[4]();
flush();
click('Следующая строка →');
assert.equal(uiMic[0], item.lines[1], 'previously read lines must be read again');
uiMic[4]();
flush();
...
assert(!uiMic[2]);
click('Повторить задание');
assert(!text(result).includes(item.question), 'repeat leaves the follow-up question');
assert.equal(uiMic[0], item.lines[0]);
uiMic[4]();
flush();
click('Следующая строка →');
uiMic[4]();
