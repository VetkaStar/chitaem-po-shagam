...
switch ($Action) {
 ...
 'check' {
  ...
  Run-Node @('tests/syllable-parts.mjs')
  Run-Node @('node_modules/typescript/bin/tsc','--noEmit')
  foreach ($test in @('regression','practice-flow','text-practice-flow','illustrations','auto-advance','curriculum','curriculum-presentation','curriculum-execution','curriculum-demonstrations','curriculum-onboarding','curriculum-roadmap','curriculum-free-practice','free-audio-flow','curriculum-completeness','curriculum-media','entry08-integration','entry08-media','entry08-personal')) { Run-Node @("tests/$test.mjs") }
